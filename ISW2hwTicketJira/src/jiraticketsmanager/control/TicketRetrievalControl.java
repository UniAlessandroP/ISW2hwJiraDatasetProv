...
import jiraticketsmanager.utils.NetUtils;
import jiraticketsmanager.entity.Ticket;

import static jiraticketsmanager.control.TicketRetrievalConstants.*;

public class TicketRetrievalControl {

    public String provideUrlForTks(String projectName, String tkType, String[] statuses, String resol, Integer startAt, Integer maxRes) {
        ...
        if(statuses != null) {
            ...
            if (statuses.length > 1) composedUrl.append(NEXT_MULT_FILTER);
            else if (statuses.length == 1) composedUrl.append(AND_C);
            
            for(String s : statuses) {
                composedUrl.append(STATUS_CLAUSE).append(D_Q).append(s).append(D_Q);
                stsize--;
                if(stsize != 0) composedUrl.append(OR_C);
            }
            if (statuses.length > 1) composedUrl.append(CLOSE_MULT_FILTER);
        }
        //Add filter by resolution
        if(resol != null && !resol.equals("")) composedUrl.append(AND_C).append(RESOLUTION_CLAUSE).append(D_Q).append(resol).append(D_Q);
